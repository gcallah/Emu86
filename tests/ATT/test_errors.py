...
class ErrorTestCase(TestCase):
    ...
    def test_invalid_instr(self):
        (output, error, bit_code) = assemble("shove_up_reg $1, %eax", 'att', intel_machine)
        self.assertTrue(error.startswith(INVALID_INSTR))

    def test_invalid_mem_loc(self):
        (output, error, bit_code) = assemble("mov $666, (hell)", 'att', intel_machine)
        self.assertTrue(error.startswith(INVALID_MEM_LOC))

    def test_invalid_num_args(self):
        (output, error, bit_code) = assemble("add $10, $22, $34, %eax", 'att', intel_machine)
